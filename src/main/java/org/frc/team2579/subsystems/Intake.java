...
public class Intake extends Subsystem implements ControlLoopable {
	...
	public Intake() {
		try {
			...
			// innerWheelPiston = new
			// DoubleSolenoid(RobotMap.INTAKE_WHEEL_IN_PCM2_ID,RobotMap.INTAKE_WHEEL_OUT_PCM2_ID);
			// intakePiston = new
			// DoubleSolenoid(RobotMap.INTAKE_DOWN_PCM_ID,RobotMap.INTAKE_UP_PCM_ID);
		} catch (Exception e) {
	...
	public double getLeftTriggerAxis() {
		return OI.getInstance().getOperatorGamepad().getLeftTriggerAxis() > 0.4
	...
	public void setSpeed() {
		 ...
		 * leftIntake.set(-0.6*(OI.getInstance().getOperatorGamepad().
		 * getRightTriggerAxis()-OI.getInstance().getOperatorGamepad().
		 * getLeftTriggerAxis()));
		 * rightIntake.set(0.6*(OI.getInstance().getOperatorGamepad().
		 * getRightTriggerAxis()-OI.getInstance().getOperatorGamepad().
		 * getLeftTriggerAxis())); }else if(!unstuck&&!rotate){
		 * leftIntake.set(-0.6*(OI.getInstance().getOperatorGamepad().
		 * getRightTriggerAxis()-OI.getInstance().getOperatorGamepad().
		 * getLeftTriggerAxis()));
		 * rightIntake.set(-0.6*(OI.getInstance().getOperatorGamepad().
