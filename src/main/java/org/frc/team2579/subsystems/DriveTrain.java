...
public class DriveTrain extends Subsystem implements ControlLoopable {
	...
	public void driveWithJoystick() {
		...
		m_moveInput = -(OI.getInstance().getDriverGamepad().getRightTriggerAxis()
				- OI.getInstance().getDriverGamepad().getLeftTriggerAxis());
		m_steerInput = OI.getInstance().getDriverGamepad().getLeftXAxis();
		/*
		 * if(m_moveInput<0) { m_steerInput =
		 * OI.getInstance().getDriverGamepad().getLeftXAxis(); }else if(m_moveInput>0) {
		 * m_steerInput = -OI.getInstance().getDriverGamepad().getLeftXAxis(); }else {
		 * m_steerInput = OI.getInstance().getDriverGamepad().getLeftXAxis(); }
		 */
		/*
		 * m_moveOutput = joystickSensitivityAdjust(m_moveInput, DRIVER_JOY1_C1,
		 * DRIVER_JOY1_C2, DRIVER_JOY1_C3); m_steerOutput =
		 * joystickSensitivityAdjust(m_steerInput, DRIVER_JOY1_C1, DRIVER_JOY1_C2,
		 * DRIVER_JOY1_C3);
		 */
	...
	public void executeDrive() {
		profileFollower.processMotionProfileBufferPeriodic();
		profileFollower.followProfilePeriodic();
	}
    ...
    {
		leftDrive1.zeroEncoder();
		rightDrive1.zeroEncoder();
    }
	...
	// DriverStation.reportError("Trajectory Duration not supported - use
	// configMotionProfileTrajectoryPeriod instead", false);
	// }
	// /* pass to caller */
	// return retval;
	// }
